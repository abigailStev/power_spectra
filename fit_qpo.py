...
__year__ = "2015"
__description__ = "Fits a QPO power spectrum (f*P(f)) with a power law and \
either a Gaussian or a Lorentzian. Not intended to be robust, just to give an \
idea of how to quantify the spectrum."
...
# 			full_output=1)
	...
	if qpo_mod == "L":
		...
		print "\tFWHM:", best_fit[1]
		Q = best_fit[0] / best_fit[1]
		## Fit Lorentzian to data
		best_qpo = lorentzian(freq, best_fit)
		best_resid = L_pl_residuals(best_fit, npn_bg_corr, freq)
		...
		best_resid = G_pl_residuals(best_fit, npn_bg_corr, freq)
	...
	make_plots(freq, npn_bg_corr, best_qpo, best_pl, best_resid, npn_err)
...
if __name__ == '__main__':
	...
	parser = argparse.ArgumentParser(usage='', description='Fits a QPO power \
spectrum (f*P(f)) with a power law and either a Gaussian or a Lorentzian. Not \
intended to be robust, just to give an idea of how to quantify the spectrum.', \
epilog='For optional arguments, default values are given in brackets at end of \
description.')
...
required=False, default='L', help='Function for QPO model: L for Lorentzian, \
G for Gaussian. [L]')
